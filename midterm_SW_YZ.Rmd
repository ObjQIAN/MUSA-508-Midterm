...
Philadelphia, often referred to as the "City of Brotherly Love," is a historic metropolis steeped in cultural significance and architectural grandeur. With its rich tapestry of neighborhoods, each boasting its own distinct character, the Philadelphia real estate market presents a unique landscape for housing price prediction. Unlike many other cities, Philadelphia's real estate dynamics are shaped by a blend of historical preservation efforts, economic diversity, and a thriving academic sector.

One defining characteristic of Philadelphia's housing market is its substantial owner-occupancy rate, which stands in contrast to cities where renting is more prevalent. This phenomenon is largely attributed to the city's strong sense of community and the enduring appeal of its historic neighborhoods. As a result, the demand for both rental and owned properties is influenced by a wide array of factors, ranging from proximity to cultural landmarks to accessibility to educational institutions.

Moreover, the interplay between the local economy and the housing market in Philadelphia is a crucial factor for accurate price prediction. The city's economy is multifaceted, with significant contributions from industries such as education, healthcare, and technology. This diversity creates a dynamic real estate environment, as shifts in employment and economic trends can have pronounced effects on housing demand and pricing.

Philadelphia's real estate landscape also bears the influence of demographic trends, including the preferences of millennials and baby boomers for urban living. This demographic shift has led to the revitalization of many neighborhoods, spurring investment in both residential and commercial properties. Additionally, initiatives aimed at urban redevelopment and sustainability have played a pivotal role in shaping the city's housing market.
...
Obviously, `crime_nn5` is the one with the strongest correlation with `sale_price`.
...
ggplot() + geom_sf(data = district, fill = "grey40") +
  ...
  mapTheme()
```
Obviously, `p311_nn5` is the one with the strongest correlation with `sale_price`.
...
ggplot() + geom_sf(data = district, fill = "grey40") +
  ...
  scale_fill_gradient(low = "#25CB10", high = "#FA7800", name = "Density") +
  scale_alpha(range = c(0.1, 0.35), guide = "none") +
  labs(title = "Density of landmarks, PHL") +
  mapTheme()
...
ggplot() + geom_sf(data = district, fill = "grey40") +
  ...
  scale_fill_gradient(low = "#25CB10", high = "#FA7800", name = "Density") +
  scale_alpha(range = c(0.1, 0.35), guide = "none") +
  labs(title = "Density of schools, PHL") +
  mapTheme()
...
### 2.1.3 Summary Statistics of Features
...
to_train <- st_join(to_train,district)
to_predict <- st_join(to_predict,district)
...
## 4.5 Predictions by neighborhood
...
## 4.6 Regression with neighborhood effects
...
reg.nhood <- lm(sale_price ~ ., data = as.data.frame(philly.training) %>% 
                                 dplyr::select(DIST_NAME.x, sale_price, total_livable_area, Age, 
                                               geographic_ward,garage_spaces,
                                               fireplaces, interior_condition,exterior_condition, number_of_bathrooms, 
                                               number_of_bedrooms, number_stories,
                                               crime_nn5,p311_nn5,landmark.Buffer,
...
## 4.7 Further examination of errors
...
st_drop_geometry(bothRegressions) %>%
  ...
  ungroup() %>% 
  left_join(district, by = c("DIST_NAME.x" = "DIST_NAME.x")) %>%
    st_sf() %>%
    ggplot() + 
...
## 4.8 Race and income context of predictions
